"""
anthropic_compat.py — Multi-turn streaming chat for Anthropic-compatible APIs.

Supports any endpoint speaking the Anthropic Messages protocol, including
DeepSeek's compatibility layer (`https://api.deepseek.com/anthropic`), with:
  - Rate-limit retry with exponential backoff (honours Retry-After header)
  - Transient error recovery (502, 503)
  - Streaming response aggregation
  - Native input/output token accounting from the usage block
"""

import re
import time

from eda_agent.llm.base import LLMResponse


class AnthropicSession:
    """Stateful multi-turn chat for Anthropic-compatible endpoints.

    The system prompt is passed as a top-level parameter (not a message), and
    history is kept as an alternating user/assistant messages list so each call
    carries the full prior context.

    Args:
        client: An Anthropic client instance (configured with base_url).
        model: Model identifier (e.g. "deepseek-v4-pro").
        system: System prompt text.
        temperature: Sampling temperature.
        max_tokens: Max output tokens per response.
    """

    def __init__(self, client, model: str, system: str,
                 temperature: float = 0.1, max_tokens: int = 16384):
        self._client = client
        self._model = model
        self._system = system
        self._temperature = temperature
        self._max_tokens = max_tokens
        self._messages: list[dict] = []

        # Reasoning models spend the whole token budget on `thinking` blocks and
        # can hit max_tokens before emitting any text, which the agent then sees
        # as an empty response. Turn thinking off for those, matching the
        # OpenAI-compatible session's behaviour for the same models.
        self._disable_thinking = any(
            x in model.lower() for x in ("deepseek", "nemotron", "qwen3")
        )

    def send_message(self, text: str) -> LLMResponse:
        """Send a user message and return the assistant's response."""
        self._messages.append({"role": "user", "content": text})
        response = self._call_with_retry()
        self._messages.append({"role": "assistant", "content": response.text})
        return response

    # ── Retry logic ───────────────────────────────────────────────────────

    def _call_with_retry(self) -> LLMResponse:
        """Retry on rate limits (indefinitely) and transient errors (up to 5x)."""
        from anthropic import RateLimitError

        rl_attempt = 0
        transient_attempt = 0

        while True:
            try:
                return self._stream_once()

            except RateLimitError as exc:
                wait = self._parse_retry_after(exc) or min(30 * (2 ** rl_attempt), 600)
                rl_attempt += 1
                print(f"\n  Rate limit (attempt {rl_attempt}) — waiting {wait:.0f}s ...")
                for remaining in range(int(wait), 0, -10):
                    print(f"  ...resuming in {remaining}s ", end="\r", flush=True)
                    time.sleep(min(10, remaining))
                print(f"  Retrying now.{'':40}")

            except Exception as exc:
                transient_attempt += 1
                if transient_attempt > 5:
                    raise
                wait = 5 * transient_attempt
                print(f"\n  API error ({transient_attempt}/5): "
                      f"{str(exc)[:120]} — waiting {wait}s...")
                time.sleep(wait)

    def _stream_once(self) -> LLMResponse:
        """Make a single streaming API call and aggregate the response."""
        chunks: list[str] = []
        in_tokens = out_tokens = 0

        kwargs = {}
        if self._disable_thinking:
            kwargs["thinking"] = {"type": "disabled"}

        with self._client.messages.stream(
            model=self._model,
            system=self._system,
            messages=self._messages,
            temperature=self._temperature,
            max_tokens=self._max_tokens,
            **kwargs,
        ) as stream:
            for text in stream.text_stream:
                chunks.append(text)
            final = stream.get_final_message()

        usage = getattr(final, "usage", None)
        if usage:
            in_tokens = getattr(usage, "input_tokens", 0) or 0
            out_tokens = getattr(usage, "output_tokens", 0) or 0

        text = "".join(chunks)
        if getattr(final, "stop_reason", None) == "max_tokens":
            # Truncated output is almost always unparseable JSON downstream —
            # say so rather than letting it look like a model failure.
            print(f"\n  Response hit max_tokens ({self._max_tokens}); "
                  "output truncated.")

        return LLMResponse(text, in_tokens, out_tokens)

    # ── Helpers ───────────────────────────────────────────────────────────

    @staticmethod
    def _parse_retry_after(exc) -> float | None:
        """Extract wait seconds from rate-limit error headers or message body."""
        # Check HTTP headers first
        hdrs = getattr(getattr(exc, "response", None), "headers", None)
        if hdrs:
            val = hdrs.get("retry-after") or hdrs.get("Retry-After")
            if val:
                try:
                    return float(val)
                except (ValueError, TypeError):
                    pass

        # Fall back to parsing the error message body
        m = re.search(r"try again in (\d+\.?\d*)\s*(ms|s|m)\b", str(exc), re.IGNORECASE)
        if m:
            amount, unit = float(m.group(1)), m.group(2).lower()
            if unit == "ms":
                return max(1.0, amount / 1000)
            if unit == "m":
                return amount * 60
            return max(1.0, amount)
        return None
